...
namespace util
{
  ...
  //!  \date     2009/03/20
  //!  $Id: HistOps.h,v 1.55 2012/07/23 11:19:19 mschrode Exp $
  class HistOps
  {
  public:
    ...
    // -------------------------------------------------------------------------------------
    static void normHist(TH1 *h, double yMax, const TString &option = "") { 
      normHist(h,option);
    ...
    // -------------------------------------------------------------------------------------
    static bool equidistLogBins(std::vector<double>& binEdges, double min, double max) {
	...
	binEdges.at(i) = pow(10., minLog + i*(maxLog-minLog)/(binEdges.size()-1));
      }
    ...
    // -------------------------------------------------------------------------------------
    static void efficiency(const TH1* hPass, const TH1* hAll, TGraphAsymmErrors* &gEff) {
      gEff->Divide(hPass,hAll,"cl=0.683 b(1,1) mode");
    }
